package com.elasticsearch.search.service.impl;

import com.elasticsearch.search.entity.City;
import com.elasticsearch.search.repository.CityRepository;
...
public class CityServiceImpl implements CityService {
    ...
    private SearchQuery getCitySearchQuery(Integer pageNumber, Integer pageSize,String searchContent) {
        ...
        FunctionScoreQueryBuilder nameFunctionScoreQueryBuilder = QueryBuilders.functionScoreQuery(QueryBuilders.matchPhraseQuery("name", searchContent),
                ScoreFunctionBuilders.weightFactorFunction(1000)).scoreMode(FunctionScoreQuery.ScoreMode.SUM).setMinScore(MIN_SCORE);

        FunctionScoreQueryBuilder descriptionFunctionScoreQueryBuilder = QueryBuilders.functionScoreQuery(QueryBuilders.matchPhraseQuery("description", searchContent),
                ScoreFunctionBuilders.weightFactorFunction(1000)).scoreMode(FunctionScoreQuery.ScoreMode.SUM).setMinScore(MIN_SCORE);
